...
if (isset($_POST['compte_rendu'])) {
    $id_userbis = $_POST['id_user'];
    //print necessary information about report that validate
    $inforeport = $pdo->query("SELECT summary_report, interest_report, comments, comments.id_user, date_appoint, report.id_client  FROM `report`, comments, appointment WHERE report.id_report=comments.id_report AND report.id_user='$id_userbis' AND report.id_appoint=appointment.id_appoint");
